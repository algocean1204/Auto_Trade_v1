...
class _HomeScreenState extends State<_HomeScreen> {
  ...
  Widget _buildTradingControlCard(BuildContext context, String Function(String) t) {
    return Consumer<TradingControlProvider>(
      builder: (context, ctrl, _) {
        ...
        final isStartingServer = ctrl.isStartingServer;
        ...
        if (isStartingServer) {
          ...
          statusBg = context.tc.primary.withValues(alpha: 0.1);
          statusIcon = Icons.dns_rounded;
          statusLabel = t('server_starting');
          hintLabel = ctrl.serverStartLog ?? t('server_starting_hint');
        } else if (!isConnected) {
          statusColor = context.tc.warning;
          statusBg = context.tc.warning.withValues(alpha: 0.1);
          statusIcon = Icons.cloud_off_rounded;
          statusLabel = t('server_offline');
          hintLabel = t('server_offline_hint');
        } else if (isRunning) {
        ...
        } else if (ctrl.isServerProtected) {
          // 매매는 끝났지만 EOD 보고서 생성을 위해 서버 보호 중
          statusColor = context.tc.primary;
          statusBg = context.tc.primary.withValues(alpha: 0.1);
          statusIcon = Icons.shield_rounded;
          statusLabel = t('server_protected');
          final until = ctrl.serverProtectedUntil;
          if (until != null) {
            final h = until.hour.toString().padLeft(2, '0');
            final m = until.minute.toString().padLeft(2, '0');
            hintLabel = '${t('server_protected_hint')} ($h:$m)';
          } else {
            hintLabel = t('server_protected_hint');
          }
        } else {
        ...
        return GlassCard(
          child: Column(
            ...
            children: [
                ...
                children: [
                  ...
                  AnimatedContainer(
                    ...
                    child: Row(
                      ...
                      children: [
                        // 실행 중/서버 시작 중일 때 점멸 애니메이션, 그 외 고정 점
                        if ((isConnected && isRunning) || isStartingServer)
              ...
              // ── 서버 중지 버튼 (이 앱이 시작한 서버 + 안전할 때만 표시) ──
              if (ctrl.serverLaunchedByUs && isConnected && ctrl.canStopServer)
                _buildServerStopButton(context, ctrl, t),
              if (ctrl.serverLaunchedByUs && isConnected && ctrl.canStopServer)
                AppSpacing.vGapSm,
  ...
  /// 서버 중지 버튼을 빌드한다.
  ///
  /// 이 앱이 시작한 서버일 때만 표시한다.
  Widget _buildServerStopButton(
        ...
        onPressed: () async {
          final confirmed = await showDialog<bool>(
            ...
            builder: (ctx) => AlertDialog(
              title: Text(t('stop_server_confirm_title')),
              content: Text(t('stop_server_confirm_msg')),
              actions: [
                TextButton(
                ...
                TextButton(
                  ...
                  child: Text(
                    t('confirm'),
                    style: TextStyle(color: context.tc.loss),
                  ),
                ),
              ],
            ),
          ...
          if (confirmed == true) {
            final stopped = await ctrl.stopServer();
            if (!stopped && context.mounted && ctrl.error != null) {
  ...
  Widget _buildPortfolioCard(BuildContext context, dynamic summary, String Function(String) t) {
    final totalPnlColor = context.tc.pnlColor(summary.totalPnl);
    final totalPnlSign = summary.totalPnl >= 0 ? '+' : '';
    final todayColor = context.tc.pnlColor(summary.todayPnl);
    final todaySign = summary.todayPnl >= 0 ? '+' : '';
    final unrealizedColor = context.tc.pnlColor(summary.unrealizedPnl);
    final unrealizedSign = summary.unrealizedPnl >= 0 ? '+' : '';
    final cumulativeColor = context.tc.pnlColor(summary.cumulativeReturn);
    final cumulativeSign = summary.cumulativeReturn >= 0 ? '+' : '';
    ...
    return GlassCard(
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: totalPnlColor.withValues(alpha: 0.12),
                  borderRadius: BorderRadius.circular(8),
                  border: Border.all(
                    color: totalPnlColor.withValues(alpha: 0.3),
                    width: 1,
                  ),
                ),
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '$totalPnlSign${fmt.format(summary.totalPnl)}',
                    ...
                    Text(
                      '$totalPnlSign${summary.totalPnlPct.toStringAsFixed(2)}%',
                      style: AppTypography.bodySmall.copyWith(
                        color: totalPnlColor,
                        fontWeight: FontWeight.w600,
                        fontSize: 11,
          ...
          AppSpacing.vGapMd,
          Divider(height: 1, color: context.tc.surfaceBorder),
          AppSpacing.vGapMd,
            ...
            children: [
              Expanded(
                child: _buildMetricTile(
                  context,
                  label: t('today_realized'),
                  value: '$todaySign${fmt.format(summary.todayPnl)}',
                  sub: '$todaySign${summary.todayPnlPct.toStringAsFixed(2)}%',
                  color: todayColor,
                ),
              ),
              AppSpacing.hGapSm,
              Expanded(
                child: _buildMetricTile(
                  context,
                  label: t('unrealized_label'),
                  value: '$unrealizedSign${fmt.format(summary.unrealizedPnl)}',
                  sub: '$unrealizedSign${summary.unrealizedPnlPct.toStringAsFixed(2)}%',
          ...
          Row(
            children: [
              Expanded(
                child: _buildMetricTile(
                  context,
                  label: t('cumulative_realized'),
                  value: '$cumulativeSign${fmt.format(summary.cumulativeReturn)}',
                  sub: null,
              ...
              Expanded(
                child: _buildMetricTile(
                  context,
                  label: t('cash'),
                  value: fmt0.format(summary.cash),
                  sub: '${summary.activePositions} ${t('positions')}',
                  color: null,
                ),
              ),
            ],
        ...
        children: [
          ...
          if (sub != null) ...[
            ...
            Text(
              sub,
              style: AppTypography.bodySmall.copyWith(
                color: color?.withValues(alpha: 0.8) ?? context.tc.textTertiary,
                fontSize: 11,
              ),
            ),
          ],
