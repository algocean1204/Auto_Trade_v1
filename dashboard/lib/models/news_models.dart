...
class NewsArticle {
  ...
  /// 시간 민감도이다. breaking/developing/analysis/background 중 하나이다.
  final String timeSensitivity;
  /// 실행 가능성이다. immediate/watch/informational 중 하나이다.
  final String actionability;
  /// 레버리지 ETF 영향 한줄 요약이다.
  final String? leveragedEtfImpact;
  ...
  const NewsArticle({
    ...
    this.timeSensitivity = 'analysis',
    this.actionability = 'informational',
    this.leveragedEtfImpact,
  });
  ...
  factory NewsArticle.fromJson(Map<String, dynamic> json) {
    ...
    // time_sensitivity 필드를 파싱한다.
    final rawTimeSensitivity = json['time_sensitivity'];
    final String timeSensitivity =
        (rawTimeSensitivity is String && rawTimeSensitivity.isNotEmpty)
            ? rawTimeSensitivity
            : 'analysis';

    // actionability 필드를 파싱한다.
    final rawActionability = json['actionability'];
    final String actionability =
        (rawActionability is String && rawActionability.isNotEmpty)
            ? rawActionability
            : 'informational';

    // leveraged_etf_impact 필드를 파싱한다.
    final rawLeveragedEtfImpact = json['leveraged_etf_impact'];
    final String? leveragedEtfImpact =
        (rawLeveragedEtfImpact is String && rawLeveragedEtfImpact.isNotEmpty)
            ? rawLeveragedEtfImpact
            : null;
    ...
    return NewsArticle(
      ...
      leveragedEtfImpact: leveragedEtfImpact,
    );
  }
  ...
  /// timeSensitivity 한국어 레이블을 반환한다.
  String get timeSensitivityLabel {
  ...
  /// actionability 한국어 레이블을 반환한다.
  String get actionabilityLabel {
  ...
  /// actionability 색상을 반환한다.
  Color get actionabilityColor {
    switch (actionability) {
      case 'immediate':
        return const Color(0xFFDC2626); // red-600
      case 'watch':
  ...
  /// 레버리지 ETF 영향이 있는지 반환한다.
  bool get hasLeveragedEtfImpact =>
      leveragedEtfImpact != null && (leveragedEtfImpact?.isNotEmpty ?? false);
