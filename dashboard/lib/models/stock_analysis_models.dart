...
class AnalysisNews {
  ...
  factory AnalysisNews.fromJson(Map<String, dynamic> json) {
    ...
    // 한국어 헤드라인 우선 선택한다. 빈 문자열은 null로 처리한다.
    final rawHeadlineKr = json['headline_kr'] as String?;
    final headlineKr =
        (rawHeadlineKr != null && rawHeadlineKr.isNotEmpty) ? rawHeadlineKr : null;
    final headline = json['headline'] as String? ?? '';
    final displayHeadline = headlineKr ?? headline;

    // summary_ko 파싱한다. 빈 문자열은 null로 처리하고, reasoning 필드로 폴백한다.
    final rawSummaryKo = json['summary_ko'] as String?;
    final rawReasoning = json['reasoning'] as String?;
    final String? summaryKo =
        (rawSummaryKo != null && rawSummaryKo.isNotEmpty)
            ? rawSummaryKo
            : (rawReasoning != null && rawReasoning.isNotEmpty)
                ? rawReasoning
                : null;
