...
def _rate_to_target_range(rate: float) -> str:
    ...
    lower = round(rate * 4) / 4  # 가장 가까운 0.25 단위로 내림
    upper = lower + 0.25
...
async def _extract_latest_value(cache: Any, key: str) -> float | None:
    """Redis 캐시에서 FRED 시리즈의 최신 값을 추출한다.
...
async def get_macro_history(
    ...
    try:
        ...
        return MacroHistoryResponse(series_id=series_id, data=data[:limit])
...
async def get_rich_macro_indicators() -> RichMacroResponse:
    ...
    try:
        cpi_val = await _extract_latest_value(cache, "macro:CPIAUCSL")
        if cpi_val is not None:
            cpi_data = {"value": round(cpi_val, 1)}
    except Exception as exc:
...
async def get_rate_outlook() -> RateOutlookResponse:
    ...
    현재 금리를 Redis 캐시(macro:DFF)에서 조회하고,
